...
namespace Hollow {
	...
	void CameraSystem::OnSceneInit()
	{
		for (int i = 0; i < mGameObjects.size(); ++i)
		{
			...
			if (pCamera->mType == CameraType::MULTI_FOCUS_CAMERA && !pCamera->mDynamicFocus)
			{
	...
	void CameraSystem::Update()
	{
		for (unsigned int i = 0; i < mGameObjects.size(); ++i)
		{
			...
			else if (pCamera->mType == CENTER_ON_ROOM_CAMERA)
			{
				UpdateCenterOnRoomCamera(pCamera, pTransform, cameraData, mGameObjects[i]);
			}
			else if (pCamera->mType == MULTI_FOCUS_CAMERA)
			{
				UpdateMultiFocusCamera(pCamera, pTransform, cameraData, mGameObjects[i]);
			}
	...
	void CameraSystem::UpdateCenterOnRoomCamera(Camera* pCamera, Transform* pTransform, CameraData& cameraData, GameObject* pGO)
	{
		// Set camera position based on object
		ScriptingManager::Instance().RunScript("CameraUpdate", pGO);
	...
	void CameraSystem::UpdateMultiFocusCamera(Camera* pCamera, Transform* pTransform, CameraData& cameraData, GameObject* pGO)
	{
