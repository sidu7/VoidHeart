...
namespace Hollow
{
	...
	void Shape::Init()
	{
		// Create buffers
		CreateBuffers();
		// Procedurally generate data for shape type
		//CreateShape("Sphere");// shapeType);
		CreateShape("Teapot");
	...
	void Shape::CreateSphere()
	{
		...
		for (int i = 0; i <= n * 2; i++)
		{
			...
			for (int j = 0; j <= n; j++)
			{
				...
				{
					// i, j, k
					mIndices.push_back((i - 1) * (n + 1) + (j - 1));
					mIndices.push_back((i - 1) * (n + 1) + j);
					mIndices.push_back((i) * (n + 1) + j);
					// i, k, l
					mIndices.push_back((i - 1) * (n + 1) + (j - 1));
					mIndices.push_back((i) * (n + 1) + j);
					mIndices.push_back((i) * (n + 1) + (j - 1));
				}
			}
		}
	}
	void Shape::CreateTeapot()
	{
		...
		for (int p = 0; p < npatches; ++p) // For each patch
		{
			for (int i = 0; i <= n; ++i) // Grid in u direction
			{
				...
				for (int j = 0; j <= n; ++j) // Grid in v direction
				{
					...
					if (i > 0 && j > 0)
					{
						mIndices.push_back(p * (n + 1) * (n + 1) + (i - 1) * (n + 1) + (j - 1));
						mIndices.push_back(p * (n + 1) * (n + 1) + (i - 1) * (n + 1) + (j));
						mIndices.push_back(p * (n + 1) * (n + 1) + (i) * (n + 1) + (j));

						mIndices.push_back(p * (n + 1) * (n + 1) + (i - 1) * (n + 1) + (j - 1));
						mIndices.push_back(p * (n + 1) * (n + 1) + (i) * (n + 1) + (j));
						mIndices.push_back(p * (n + 1) * (n + 1) + (i) * (n + 1) + (j - 1));
					}
				}
			}
		}
